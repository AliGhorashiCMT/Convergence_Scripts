...
"""
Provide a filebase, e.g. BN33BC if the filename is BN33BC.ionpos, two atomic positions, and this will output the corresponding bilayer ionic positions
into a file in the same directory with the filename \$filebaseBilayer.ionpos
"""
function write_bilayer(filebase::String, atom1::Int, atom2::Int)
	np = pyimport("numpy")
	atomic_ids = String[]
	for line in readlines(filebase*".ionpos")
		push!(atomic_ids, string.(split(line))[2])
	end
	monolayer_positions = np.loadtxt(filebase*".ionpos", usecols=(2, 3, 4))
	open(filebase*"Bilayer.ionpos", write=true, create=true) do io 
		for (id, atom) in zip(atomic_ids, monolayer_positions)
			write(io, "ion ", id, [string(a, " ") for a in atom]...)
 		end
	end
end
